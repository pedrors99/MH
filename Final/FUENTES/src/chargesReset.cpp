...
/*
 * chargesMaxIter: Variante de la metaheurística basada en cargas eléctricas, que se reinicia todas las soluciones menos la
 * mejor sustituyendolas por soluciones aleatorias tras un número de iteraciones sin mejora.
 *
 ...
 * @maxIter: int, número máximo de iteraciones sin mejora tras las que reiniciar las soluciones.
 *
 * return: vector<float>, vector de pesos solución.
 */
vector<float> chargesReset(vector<vector<float> > x, vector<string> y, int chromosomes, int evaluations, float reduction, int maxIter) {
	...
	int bestOldSolution = 0;
	int noChange = -1;

	while (eval < evaluations) {
		if (noChange == maxIter) {
			solutions[0] = solutions[bestOldSolution];
			fitness[0] = fitness[bestOldSolution];

			for (int i=1; i<chromosomes; i++) {
				solutions[i] = mutation(solutions[0], 0.1, reduction, generator, normal_distribution);
		...
		if (bestOldSolution == bestSolution) {
			noChange++;
		} else {
			noChange = 0;
			bestOldSolution = bestSolution;
		}

		for (int i=0; i<chromosomes; i++) {
			if (i != bestSolution) {
				float pullForce = bestFitness * fitness[i] * (1 - (bestFitness - fitness[i]) * (bestFitness - fitness[i]));
				float pushForce = worstSolution * fitness[i] * (1 - (worstFitness - fitness[i]) * (worstFitness - fitness[i]));
...
/*
 * exportChargesReset: Variante de la metaheurística basada en cargas eléctricas, que se reinicia todas las soluciones menos la
 * mejor sustituyendolas por soluciones aleatorias tras un número de iteraciones sin mejora, exportando los resultados a un
 * fichero txt para ser representados.
 *
 * @x: vector<vector<float> >, matriz de datos.
 * @y: vector<string>, vector de etiquetas reales.
 * @chromosomes: int, número de cromosomas o soluciones iniciales.
 ...
 * return: vector<float>, vector de pesos solución.
 */
vector<float> exportChargesReset(vector<vector<float> > x, vector<string> y, int chromosomes, int evaluations, float reduction, int maxIter, int fold, string file) {
	// Abrimos o creamos (si no existe) el archivo para guardar los datos
	string filename = "graph/" + file + to_string(fold+1) + "-chargesReset.txt";
	regex target(".arff");
...
int main(int argc, char** argv) {
	...
	for (int k=0; k<FOLDS; k++) {
		...
		if (EXPORT) {
			w = exportChargesReset(x, y, CHROMOSOMES, EVALUATIONS, REDUCCTION, MAXITER, k, argv[1]);
		} else {
			w = chargesReset(x, y, CHROMOSOMES, EVALUATIONS, REDUCCTION, MAXITER);
		}
