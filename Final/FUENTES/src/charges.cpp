...
/*
 * charges: Metaheurística basada en cargas eléctricas.
 *
 ...
 */
vector<float> charges(vector<vector<float> > x, vector<string> y, int chromosomes, int evaluations, float reduction) {
...
/*
 * exportCharges: Metaheurística basada en cargas eléctricas, exportando los resultados a un fichero txt para ser representados.
 *
 * @x: vector<vector<float> >, matriz de datos.
 * @y: vector<string>, vector de etiquetas reales.
 * @chromosomes: int, número de cromosomas o soluciones iniciales.
 ...
 * return: vector<float>, vector de pesos solución.
 */
vector<float> exportCharges(vector<vector<float> > x, vector<string> y, int chromosomes, int evaluations, float reduction, int fold, string file) {
	// Abrimos o creamos (si no existe) el archivo para guardar los datos
	string filename = "graph/" + file + to_string(fold+1) + "-charges.txt";
	regex target(".arff");
...
int main(int argc, char** argv) {
	...
	for (int k=0; k<FOLDS; k++) {
		...
		if (EXPORT) {
			w = exportCharges(x, y, CHROMOSOMES, EVALUATIONS, REDUCCTION, k, argv[1]);
		} else {
			w = charges(x, y, CHROMOSOMES, EVALUATIONS, REDUCCTION);
		}
