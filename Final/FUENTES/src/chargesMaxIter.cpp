...
/*
 * chargesMaxIter: Variante de la metaheurística basada en cargas eléctricas, que se detiene tras un número de
 * iteraciones sin encontrar mejoras.
 *
 ...
 * @maxIter: int, número máximo de iteraciones sin mejora.
 *
 * return: vector<float>, vector de pesos solución.
 */
vector<float> chargesMaxIter(vector<vector<float> > x, vector<string> y, int chromosomes, int evaluations, float reduction, int maxIter) {
	...
	while (eval < evaluations && noChange < maxIter) {
		int bestSolution = 0;
		int worstSolution = 0;
...
/*
 * exportChargesMaxIter: Variante de la metaheurística basada en cargas eléctricas, que se detiene tras un número de
 * iteraciones sin encontrar mejoras, exportando los resultados a un fichero txt para ser representados.
 *
 * @x: vector<vector<float> >, matriz de datos.
 * @y: vector<string>, vector de etiquetas reales.
 * @chromosomes: int, número de cromosomas o soluciones iniciales.
 ...
 * return: vector<float>, vector de pesos solución.
 */
vector<float> exportChargesMaxIter(vector<vector<float> > x, vector<string> y, int chromosomes, int evaluations, float reduction, int maxIter, int fold, string file) {
	// Abrimos o creamos (si no existe) el archivo para guardar los datos
	string filename = "graph/" + file + to_string(fold+1) + "-chargesMaxIter.txt";
	regex target(".arff");
...
int main(int argc, char** argv) {
	...
	for (int k=0; k<FOLDS; k++) {
		...
		if (EXPORT) {
			w = exportChargesMaxIter(x, y, CHROMOSOMES, EVALUATIONS, REDUCCTION, MAXITER, k, argv[1]);
		} else {
			w = chargesMaxIter(x, y, CHROMOSOMES, EVALUATIONS, REDUCCTION, MAXITER);
		}
