...
/*
 * chargesBl: Variante de la metaheurística basada en cargas eléctricas, que explora el entorno de la mejor solución
 * tras cada iteracion.
 *
 ...
 */
vector<float> chargesBl(vector<vector<float> > x, vector<string> y, int chromosomes, int evaluations, float reduction) {
	...
	while (eval < evaluations) {
		...
		solutions[bestSolution] = memeticLocalSearch(x, y, solutions[bestSolution], solutions[bestSolution].size(), fitness[bestSolution], eval, evaluations, reduction, generator, normal_distribution);
...
/*
 * exportChargesBl: Variante de la metaheurística basada en cargas eléctricas, que explora el entorno de la mejor solución
 * tras cada iteracion, exportando los resultados a un fichero txt para ser representados.
 *
 * @x: vector<vector<float> >, matriz de datos.
 * @y: vector<string>, vector de etiquetas reales.
 * @chromosomes: int, número de cromosomas o soluciones iniciales.
 ...
 * return: vector<float>, vector de pesos solución.
 */
vector<float> exportChargesBl(vector<vector<float> > x, vector<string> y, int chromosomes, int evaluations, float reduction, int fold, string file) {
	// Abrimos o creamos (si no existe) el archivo para guardar los datos
	string filename = "graph/" + file + to_string(fold+1) + "-chargesBl.txt";
	regex target(".arff");
...
int main(int argc, char** argv) {
	...
	for (int k=0; k<FOLDS; k++) {
		...
		if (EXPORT) {
			w = exportChargesBl(x, y, CHROMOSOMES, EVALUATIONS, REDUCCTION, k, argv[1]);
		} else {
			w = chargesBl(x, y, CHROMOSOMES, EVALUATIONS, REDUCCTION);
		}
