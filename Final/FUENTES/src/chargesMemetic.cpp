...
/*
 * chargesMemetic: Variante de la metaheurística basada en cargas eléctricas, que utiliza hibridación con la búsqueda local.
 *
 ...
 * @memeticStep: int, número de iteraciones tras las cuales aplicar la búsqueda local.
 ...
 */
vector<float> chargesMemetic(vector<vector<float> > x, vector<string> y, int chromosomes, int evaluations, int memeticStep, float reduction) {
	...
	while (eval < evaluations) {
		...
		if (generation % memeticStep == 0) {
			int memeticSize = size * chromosomes;
			vector<int> ind = permutation(solutions.size());

			for (int i=0; i<memeticSize; i++) {
				solutions[ind[i]] = memeticLocalSearch(x, y, solutions[ind[i]], 2 * solutions[ind[i]].size(), fitness[i], eval, evaluations, reduction, generator, normal_distribution);
			}
		}
	}
...
/*
 * exportChargesMemetic: Variante de la metaheurística basada en cargas eléctricas, que utiliza hibridación con la búsqueda
 * local, exportando los resultados a un fichero txt para ser representados.
 *
 * @x: vector<vector<float> >, matriz de datos.
 * @y: vector<string>, vector de etiquetas reales.
 * @chromosomes: int, número de cromosomas o soluciones iniciales.
 ...
 * return: vector<float>, vector de pesos solución.
 */
vector<float> exportChargesMemetic(vector<vector<float> > x, vector<string> y, int chromosomes, int evaluations, int memeticStep, float reduction, int fold, string file) {
	// Abrimos o creamos (si no existe) el archivo para guardar los datos
	string filename = "graph/" + file + to_string(fold+1) + "-chargesMemetic.txt";
	regex target(".arff");
...
int main(int argc, char** argv) {
	...
	for (int k=0; k<FOLDS; k++) {
		...
		if (EXPORT) {
			w = exportChargesMemetic(x, y, CHROMOSOMES, EVALUATIONS, MEMETICSTEP, REDUCCTION, k, argv[1]);
		} else {
			w = chargesMemetic(x, y, CHROMOSOMES, EVALUATIONS, MEMETICSTEP, REDUCCTION);
		}
