...
/*
 * aggBlx: Algortimo genético generacional con elitismo con cruce blx.
 *
 ...
 * @cross_chance: float, probabilidad de cruce, entre 0 y 1.
 * @mutation_chance: float, probabilidad de mutación, entre 0 y 1.
 *
 * return: vector<float>, vector de pesos solución.
 */
vector<float> aggBlx(vector<vector<float> > x, vector<string> y, int chromosomes, int iterations, float reduction, float alpha, float crossChance, float mutationChance) {
	...
	while (eval < iterations) {
		...
		// Cruce
		newSolutions = crossBlx(newSolutions, crossChance, alpha, reduction, generator, uniform_distribution);
...
/*
 * aggBlx: Algortimo genético generacional con elitismo con cruce blx exportando los resultados.
 *
 * @x: vector<vector<float> >, matriz de datos.
 * @y: vector<string>, vector de etiquetas reales.
 * @chromosomes: int, número de cromosomas.
 ...
 * return: vector<float>, vector de pesos solución.
 */
vector<float> exportAggBlx(vector<vector<float> > x, vector<string> y, int chromosomes, int iterations, float reduction, float alpha, float crossChance, float mutationChance, int fold, string file) {
	// Abrimos o creamos (si no existe) el archivo para guardar los datos
	string filename = "graph/" + file + to_string(fold+1) + "-aggBlx.txt";
	regex target(".arff");
...
int main(int argc, char** argv) {
	...
	for (int k=0; k<FOLDS; k++) {
		...
		if (EXPORT) {
			w = exportAggBlx(x_train, y_train, CHROMOSOMES, ITERATIONS, REDUCCTION, ALPHA, CROSS, MUTATION, k, argv[1]);
		} else {
			w = aggBlx(x_train, y_train, CHROMOSOMES, ITERATIONS, REDUCCTION, ALPHA, CROSS, MUTATION);
		}
