...
/*
 * aggCa: Algortimo genético generacional con elitismo con aritmético aritmético.
 *
 ...
 */
vector<float> aggCa(vector<vector<float> > x, vector<string> y, int chromosomes, int iterations, float reduction, float crossChance, float mutationChance) {
...
/*
 * exportAggCa: Algortimo genético generacional con elitismo con aritmético blx exportando los resultados.
 *
 * @x: vector<vector<float> >, matriz de datos.
 * @y: vector<string>, vector de etiquetas reales.
 * @chromosomes: int, número de cromosomas.
 ...
 * return: vector<float>, vector de pesos solución.
 */
vector<float> exportAggCa(vector<vector<float> > x, vector<string> y, int chromosomes, int iterations, float reduction, float crossChance, float mutationChance, int fold, string file) {
	// Abrimos o creamos (si no existe) el archivo para guardar los datos
	string filename = "graph/" + file + to_string(fold+1) + "-aggCa.txt";
	regex target(".arff");
...
int main(int argc, char** argv) {
	...
	for (int k=0; k<FOLDS; k++) {
		...
		if (EXPORT) {
			w = exportAggCa(x_train, y_train, CHROMOSOMES, ITERATIONS, REDUCCTION, CROSS, MUTATION, k, argv[1]);
		} else {
			w = aggCa(x_train, y_train, CHROMOSOMES, ITERATIONS, REDUCCTION, CROSS, MUTATION);
		}
