...
/*
 * ageCa: Algortimo genético estacionario con cruce aritmético.
 *
 ...
 */
vector<float> ageCa(vector<vector<float> > x, vector<string> y, int chromosomes, int iterations, float reduction, float alpha, float mutationChance) {
	...
	while (eval < iterations) {
		...
		// Cruce
		newSolutions = crossCa(newSolutions, 1.0f, reduction, generator, uniform_distribution);
...
/*
 * exportAgeCa: Algortimo genético estacionario con cruce aritmético exportando resultados.
 *
 * @x: vector<vector<float> >, matriz de datos.
 * @y: vector<string>, vector de etiquetas reales.
 * @chromosomes: int, número de cromosomas.
 ...
 * return: vector<float>, vector de pesos solución.
 */
vector<float> exportAgeCa(vector<vector<float> > x, vector<string> y, int chromosomes, int iterations, float reduction, float alpha, float mutationChance, int fold, string file) {
	// Abrimos o creamos (si no existe) el archivo para guardar los datos
	string filename = "graph/" + file + to_string(fold+1) + "-ageCa.txt";
	regex target(".arff");
...
int main(int argc, char** argv) {
	...
	for (int k=0; k<FOLDS; k++) {
		...
		if (EXPORT) {
			w = exportAgeCa(x_train, y_train, CHROMOSOMES, ITERATIONS, REDUCCTION, CROSS, MUTATION, k, argv[1]);
		} else {
			w = ageCa(x_train, y_train, CHROMOSOMES, ITERATIONS, REDUCCTION, CROSS, MUTATION);
		}
