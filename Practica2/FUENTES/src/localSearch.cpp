...
/*
 * exportLocalSearch: Algortimo de búsqueda local exportando los resultados.
 *
 * @x: vector<vector<float> >, matriz de datos.
 * @y: vector<string>, vector de etiquetas reales.
 * @evaluations: int, número de evaluaciones.
 ...
 * return: vector<float>, vector de pesos solución.
 */
vector<float> exportLocalSearch(vector<vector<float> > x, vector<string> y, int evaluations, int maxNeighbours, float reduction, int fold, string file) {
	// Abrimos o creamos (si no existe) el archivo para guardar los datos
	string filename = "graph/" + file + to_string(fold+1) + "-localSearch.txt";
	regex target(".arff");
	...
	w = exportLocalSearch(x, y, w, reduction, eval, evaluations, maxNeighbours, generator, normal_distribution, myfile);
...
int main(int argc, char** argv) {
	...
	for (int k=0; k<FOLDS; k++) {
		...
		if (EXPORT) {
			w = exportLocalSearch(x_train, y_train, ITERATIONS, MAXNEIGHBOURS, REDUCCTION, k, argv[1]);
		} else {
