...
/*
 * bmb: Algortimo de búsqueda multiarranque básica.
 *
 ...
 */
vector<float> bmb(vector<vector<float> > x, vector<string> y, int iterations, int evaluations, int maxNeighbours, float reduction) {
	...
	for (int i=0; i<iterations; i++) {
		w = initialize(x[0].size(), reduction, generator, uniform_distribution);
		int eval = 0;
		w = localSearch(x, y, w, reduction, eval, evaluations, maxNeighbours, generator, normal_distribution);

		float agre = agregation(x, y, w, reduction);
...
int main(int argc, char** argv) {
	...
	for (int k=0; k<FOLDS; k++) {
		...
		w = bmb(x, y, ITERATIONS, EVALUATIONS, MAXNEIGHBOURS, REDUCCTION);
