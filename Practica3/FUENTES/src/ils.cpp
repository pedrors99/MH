...
int ITERATIONS = 15; /* Número de veces que se realiza la búsqueda local */
int EVALUATIONS = 1000; /* Número de evaluaciones para cada búsqueda local */
int MAXNEIGHBOURS = 20; /* Número máximo de vecinos que explora hasta detenerse si no encuentra mejora */
float MUTATIONCHANCE = 0.1f; /* Probabilidad de mutación */
...
/*
 * ils: Algortimo de búsqueda local reiterada.
 *
 * @x: vector<vector<float> >, matriz de datos.
 * @y: vector<string>, vector de etiquetas reales.
 * @evaluations: int, número de evaluaciones.
 * @maxNeighbours: int, número máximo de vecinos explorados en la búsqueda local.
 * @mutation: float, porcentage de elementos que modificamos trás cada búsqueda local.
 * @reduction: float, cota para la tasa de reducción.
 *
 * return: vector<float>, vector de pesos solución.
 */
vector<float> ils(vector<vector<float> > x, vector<string> y, int iterations, int evaluations, int maxNeighbours, float mutationChance, float reduction) {
	default_random_engine generator(SEED);
	uniform_real_distribution<float> uniform_distribution(0.0, 1.0);
	normal_distribution<float> normal_distribution1(0.0, 0.3);
	normal_distribution<float> normal_distribution2(0.0, 0.4);

	vector<float> w = initialize(x[0].size(), reduction, generator, uniform_distribution);
	int eval = 0;

	w = localSearch(x, y, w, reduction, eval, evaluations, maxNeighbours, generator, normal_distribution1);
	float fitness = agregation(x, y, w, reduction);

	vector<float> bestSolution = w;
	float bestFitness = fitness;

	for (int i=1; i<iterations; i++) {
		w = mutation(bestSolution, mutationChance, reduction, generator, normal_distribution2);

		eval = 0;
		w = localSearch(x, y, w, reduction, eval, evaluations, maxNeighbours, generator, normal_distribution1);
		fitness = agregation(x, y, w, reduction);
...
int main(int argc, char** argv) {
	...
	for (int k=0; k<FOLDS; k++) {
		...
		w = ils(x_train, y_train, ITERATIONS, EVALUATIONS, MAXNEIGHBOURS, MUTATIONCHANCE, REDUCCTION);
