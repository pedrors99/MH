...
float MUTATIONCHANCE = 0.1; /* Probabilidad de mutación */
...
/*
 * ils-es: Algortimo de búsqueda local reiterada utilizando enfriamiento simulado.
 *
 ...
 */
vector<float> ils_es(vector<vector<float> > x, vector<string> y, int iterations, int evaluations, float reduction, float mutationChance, float mu, float phi) {
	...
	w = es(w, x, y, reduction, mu, phi, eval, evaluations, generator, normal_distribution1, uniform_distribution);
...
int main(int argc, char** argv) {
	...
	for (int k=0; k<FOLDS; k++) {
		...
		w = ils_es(x_train, y_train, ITERATIONS, EVALUATIONS, REDUCCTION, MUTATIONCHANCE, MU, PHI);
